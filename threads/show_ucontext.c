...
// 	       "in mycontext = 0x%lx\n", (long int)mycontext.uc_mcontext.gregs[REG_RIP]);

// 	/* now, think about parameters */
// 	// printf("argc = %d\n", (int)mycontext.uc_mcontext.gregs[REG_RDI]); //1st arg is argc so stored in rdi 
// 	// printf("argv = %p\n", (void *)mycontext.uc_mcontext.gregs[REG_RSI]); //2nd arg is argv so in rsi

// 	printf("argc = %d\n", (int)argc); //1st arg is argc so stored in rdi 
// 	printf("argv = %p\n", (void *)argv);
...
// 	//        (void *)mycontext.uc_mcontext.gregs[REG_RBP] - 20); 
// 	printf("memory address of the variable setcontext_called = %p\n",
// 		(void *)&setcontext_called); 
// 	// the rbp pts to the top of the stack rn and bc setcontext called after end and start (both long which is 8 bytes each + the value of int = 20)
// 	// similarly below err is the next thing called so - 24
// 	// printf("memory address of the variable err = %p\n",
// 	//        (void *)mycontext.uc_mcontext.gregs[REG_RBP] -24);
// 	printf("memory address of the variable err = %p\n",
// 		(void *)&err);
// 	printf("number of bytes pushed to the stack between setcontext_called "
// 	       "and err = %ld\n", (unsigned long)(&setcontext_called - &err));
// 			// lol this is cheating since we have access but will return the same as above should only be an 8 byte difference
...
// 	       "= %ld\n", (unsigned long int)((long long int)&err - mycontext.uc_mcontext.gregs[REG_RSP]));
...
main(int argc, char **argv)
{
	...
	printf("number of bytes between err and the saved stack in mycontext "
	       "= %ld\n", (unsigned long) (&err - (int*)mycontext.uc_mcontext.gregs[REG_RSP]));
